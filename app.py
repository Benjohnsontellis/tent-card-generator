    ...
    if excel_file is None:
        ...
        with st.spinner("Generating PDF(s)..."):
            for sheet in xls.sheet_names:
                ...
                generated.append((sheet, pdf_path))

        st.success(f"✅ {len(generated)} PDF(s) generated!")

        for sheet_name, path in generated:
            with open(path, "rb") as f:
                st.download_button(
                    label=f"⬇️ Download — {sheet_name}.pdf",
                    data=f,
                    file_name=f"tent_cards_{sheet_name}.pdf",
