...
# ── 2. SHEET SELECTOR (shows only after file is uploaded) ────
selected_sheets = []
if excel_file:
    ...
    select_all = st.checkbox("Select All Sheets", value=True)

    if select_all:
        selected_sheets = all_sheets
        st.info(f"✅ All {len(all_sheets)} sheet(s) selected: {', '.join(all_sheets)}")
    else:
        selected_sheets = st.multiselect(
            "Choose which sheets to generate tent cards for:",
            options=all_sheets,
...
if st.button("⚙️ Generate Tent Cards", type="primary", use_container_width=True):
    ...
    if excel_file is None:
        ...
        with st.spinner(f"Generating {len(selected_sheets)} sheet(s)..."):
            for sheet in selected_sheets:
                ...
                generated.append((sheet, out_path))

        st.success(f"✅ {len(generated)} file(s) generated!")

        # ── Single sheet — direct download ───────────────────
        if len(generated) == 1:
            sheet_name, path = generated[0]
        ...
        # ── Multiple sheets — ZIP download ───────────────────
        else:
            zip_buffer = io.BytesIO()
            with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
                for sheet_name, path in generated:
                    zf.write(path, arcname=f"tent_cards_{sheet_name}.{ext}")
            zip_buffer.seek(0)

            st.download_button(
                label=f"⬇️ Download All ({len(generated)} files) as ZIP",
                data=zip_buffer,
                file_name=f"tent_cards_all.zip",
                mime=mime_zip,
                use_container_width=True
            )

            # Also show individual downloads below
