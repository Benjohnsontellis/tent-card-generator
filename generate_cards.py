    ...
    for _, row in df.iterrows():
        ...
        if login_data:
            ...
            if n_boxes == 1:
                ...
                # Three boxes — evenly spread across full page width
                u0, p0 = login_data[0]
                u1, p1 = login_data[1]
                u2, p2 = login_data[2]
                GAP_COL = 8
                BOX_W3  = (PAGE_W - 2 * GAP_COL) / 3
                cw     = [BOX_W3, GAP_COL, BOX_W3, GAP_COL, BOX_W3]
                row1   = [
                ...
                login_table.setStyle(TableStyle([
                    ...
                    ('LEFTPADDING',   (0, 0), (-1, -1), 0),  # zero all first
                    ('RIGHTPADDING',  (0, 0), (-1, -1), 0),
                    ('LEFTPADDING',   (0, 0), (0, -1), 8),   # then set box cols
                    ('RIGHTPADDING',  (0, 0), (0, -1), 8),
                    ('LEFTPADDING',   (2, 0), (2, -1), 8),
                    ('RIGHTPADDING',  (2, 0), (2, -1), 8),
        ...
        for n in patients:
            ...
            excel_label = clean(row.get(f"Patient{n}Label"))
            if excel_label:
                pat_label = excel_label.strip()
        ...
        for i in range(1, 15):
            ...
            if med_name and ndc:
                ...
                cell = Table([
                    [Paragraph(f"{med_name}", bold_center)],
                    [qr_img]
                ], colWidths=[PAGE_W / 2 - 0.2 * inch], rowHeights=[0.35 * inch, 0.95 * inch])
        ...
        if meds:
            ...
            # Build grid with name row and QR row as separate outer rows
            med_pairs = []
            for i in range(0, len(meds), 2):
                pair = meds[i:i+2]
                while len(pair) < 2:
                    pair.append(None)
                med_pairs.append(pair)

            for idx, (left, right) in enumerate(med_pairs):
                left_name  = left._cellvalues[0][0]  if left  else Paragraph("", small)
                left_qr    = left._cellvalues[1][0]  if left  else Paragraph("", small)
                right_name = right._cellvalues[0][0] if right else Paragraph("", small)
                right_qr   = right._cellvalues[1][0] if right else Paragraph("", small)

                pair_grid = [
                    [left_name,  right_name],
                    [left_qr,    right_qr],
                ]
                pair_table = Table(pair_grid,
                                   colWidths=[PAGE_W / 2, PAGE_W / 2],
                                   rowHeights=[0.35 * inch, 0.95 * inch],
                                   splitByRow=False)
                pair_table.setStyle(TableStyle([
                    ('ALIGN',         (0, 0), (-1, -1), 'CENTER'),
                    ('VALIGN',        (0, 0), (-1, -1), 'MIDDLE'),
                    ('BOX',           (0, 0), (-1, -1), 0.75, colors.HexColor('#aaaaaa')),
                    ('INNERGRID',     (0, 0), (-1, -1), 0.75, colors.HexColor('#aaaaaa')),
                    ('TOPPADDING',    (0, 0), (-1, -1), 4),
                    ('BOTTOMPADDING', (0, 0), (-1, -1), 4),
                    ('LEFTPADDING',   (0, 0), (-1, -1), 4),
                    ('RIGHTPADDING',  (0, 0), (-1, -1), 4),
                ]))
                # Keep heading with first pair so it never strands at bottom of page
                if idx == 0:
                    elements.append(KeepTogether([
                        Paragraph("<b>Medications</b>", bold_center),
                        Spacer(1, 6),
                        pair_table
                    ]))
                else:
        ...
        for i in range(1, 10):
            ...
            if specimen and barcode:
                ...
                cell = Table([
                    [Paragraph(f"{specimen}", bold_center)],
                    [qr_img]
                ], colWidths=[PAGE_W / 2 - 0.2 * inch], rowHeights=[0.35 * inch, 0.95 * inch])
        ...
        if specimens:
            ...
            spec_pairs = []
            for i in range(0, len(specimens), 2):
                pair = specimens[i:i+2]
                while len(pair) < 2:
                    pair.append(None)
                spec_pairs.append(pair)

            for idx, (left, right) in enumerate(spec_pairs):
                left_name  = left._cellvalues[0][0]  if left  else Paragraph("", small)
                left_qr    = left._cellvalues[1][0]  if left  else Paragraph("", small)
                right_name = right._cellvalues[0][0] if right else Paragraph("", small)
                right_qr   = right._cellvalues[1][0] if right else Paragraph("", small)
                ...
                if idx == 0:
                    elements.append(KeepTogether([
                        Paragraph("<b>Specimens</b>", bold_center),
                        Spacer(1, 6),
                        pair_table
                    ]))
                else:
